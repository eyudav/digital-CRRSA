...
import { Bell, FileText, Inbox, LayoutDashboard, LogOut, Megaphone, MessageSquareWarning, ScrollText, Sparkles, Calendar, Settings, Menu, User } from "lucide-react";
...
export function AppShell() {
    ...
    const userInitials = user.fullName.split(" ").map((p) => p[0]).slice(0, 2).join("");

    return (
      <SidebarProvider>
        <div className="flex min-h-screen w-full bg-background transition-colors duration-300">
          <Sidebar variant="inset" className="border-r border-border bg-sidebar shadow-elegant">
            ...
            <SidebarFooter className="p-4">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="w-full h-12 justify-start gap-3 px-2 hover:bg-sidebar-accent">
                    ...
                    <div className="flex flex-col items-start min-w-0 flex-1 overflow-hidden">
                      ...
                      <span className="text-xs text-muted-foreground capitalize truncate w-full">{user.role.replace("_", " ")}</span>
                    </div>
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => navigate(`/${user.role === "citizen" ? "citizen" : "staff"}/settings`)}>
                    <User className="mr-2 h-4 w-4" /> Profile settings
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => { logout(); navigate("/"); }} className="text-destructive focus:text-destructive focus:bg-destructive/10">
