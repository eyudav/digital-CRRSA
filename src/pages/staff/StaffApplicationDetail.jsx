...
const StaffApplicationDetail = () => {
    ...
    return (
      <div className="p-8">
        <p className="text-sm text-destructive">
          {error?.message || "Could not load application."}
        </p>
        <Button
          variant="outline"
          size="sm"
  ...
  const requestDocs = () => {
    ...
    patchStatus.mutate(
      { statusUi: "additional_documents_required", message: comment },
      {
  ...
  const addComment = () => {
    ...
    patchStatus.mutate(
      { statusUi: app.status, message: comment },
      {
        onSuccess: () => setComment(""),
      },
    );
  ...
  const canMarkReady = ["approved", "scheduled"].includes(app.status);

  const issueCertificate = () => {
    if (!canMarkReady) {
      toast({
        title: "Approve application first",
        description:
          "You must approve the application before issuing or marking it ready for collection.",
        variant: "destructive",
      });
      return;
    }
    setStatus(
      ...
      "Certificate issued and ready for collection.",
    );
  };
  ...
  return (
    <>
      ...
      <PageHeader
        eyebrow={app.refNumber}
        title={`${app.serviceName} — ${app.citizenName}`}
        description={`Submitted ${format(new Date(app.submittedAt), "MMMM d, yyyy")}`}
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <div className="space-y-5 lg:col-span-2">
              ...
              {Object.entries(app.formData).map(([k, v]) => {
                ...
                if (typeof v === "object" && v !== null) {
                  return (
                    <div
                      key={k}
                      className="md:col-span-2 bg-secondary/15 p-3 rounded-xl border border-border/40 mt-1"
                    >
          ...
          {app.formData.appointmentMetadata && (
            <ContentCard title="Appointment Confirmations" icon={UserCheck}>
              <dl className="grid gap-4 text-sm md:grid-cols-2">
                {Object.entries(app.formData.appointmentMetadata).map(
                  ([k, v]) => {
          ...
          <ContentCard title="Documents" icon={Upload}>
            {app.documents.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Citizen has not uploaded documents yet.
              </p>
            ) : (
          ...
          <ContentCard
            title="Add comment / request documents"
            icon={MessageSquare}
          >
            <Textarea
              rows={3}
              placeholder="Write a note to the citizen or an internal review comment..."
              value={comment}
            ...
            <ol className="relative ml-3 space-y-5 border-l border-border pl-6">
              {app.timeline.map((t) => (
                <li key={t.id} className="relative">
                  <span className="absolute -left-[31px] grid h-5 w-5 place-items-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground">
                    ●
                  </span>
                  <p className="text-sm font-medium">{t.message}</p>
                  <p className="text-xs text-muted-foreground">
                    {format(new Date(t.at), "MMM d, yyyy · HH:mm")} · {t.by}
                  </p>
                </li>
              ))}
            </ol>
        ...
        <div className="space-y-5">
          <ContentCard title="Decision" icon={Check}>
            <p className="text-sm text-muted-foreground">
              Mark this application's outcome. Citizens are notified
              automatically.
            </p>
            <div className="mt-4 grid gap-2">
              <Button
                onClick={() => setStatus("approved", "Application approved.")}
              ...
              <Button
                ...
                disabled={patchStatus.isPending || !canMarkReady}
                title={
                  !canMarkReady
                    ? "Approve the application before marking ready for collection"
                    : undefined
                }
              >
                Issue & mark ready
              ...
              <Button
                onClick={() => setStatus("rejected", "Application rejected.")}
                variant="outline"
          ...
          {app.appointment && (
            <ContentCard title="Appointment" icon={ScrollText}>
              <p className="font-display text-base font-semibold">
                {app.appointment.office}
              </p>
              <p className="mt-1 text-sm text-muted-foreground">
                {format(new Date(app.appointment.date), "EEEE, MMMM d")} ·{" "}
                {app.appointment.timeSlot}
              </p>
              <p className="mt-2 text-xs text-muted-foreground">
                Queue #{app.appointment.queueNumber}
              </p>
            </ContentCard>
          )}
        </div>
