...
const CitizenDashboard = () => {
  ...
  const inProgress = myApps.filter(
    (a) => !["rejected"].includes(a.status),
  ).length;
  const ready = myApps.filter(
    (a) => a.status === "ready_for_collection",
  ).length;
  ...
  return (
    <>
      <PageHeader
        eyebrow={`Hello, ${user?.fullName.split(" ")[0]}`}
        title="Your civil services at a glance"
        description="Track applications, manage appointments and stay in the loop on every notification."
        actions={
          <Button
            asChild
          ...
          >
            <Link to="/citizen/services">
              <Sparkles className="mr-1.5 h-4 w-4" /> Start a new application
            </Link>
          </Button>
        }
      />
      ...
      {appsLoading ? (
        ...
        <>
          ...
          <DashboardAnalytics apps={myApps} />
            ...
            <div className="rounded-2xl border border-border bg-card p-6 shadow-soft lg:col-span-2">
              ...
              {myApps.length === 0 ? (
                <EmptyState
                  ...
                  description="Start your first application to see real-time status updates here."
                  action={
                    <Button asChild>
                      <Link to="/citizen/services">Browse services</Link>
                    </Button>
                  }
                />
              ) : (
                  ...
                  {myApps.slice(0, 5).map((a) => (
                    <li key={a.id}>
                      <Link
                        ...
                        className="flex items-center justify-between py-3 transition-colors hover:bg-secondary/40 -mx-3 px-3 rounded-lg"
                      >
                ...
                {upcoming?.appointment ? (
                  <div className="mt-3">
                    <p className="font-display text-base font-semibold">
                      {upcoming.appointment.office}
                    </p>
                    <p className="mt-1 text-sm text-muted-foreground">
                      {format(
                        new Date(upcoming.appointment.date),
                        "EEEE, MMMM d",
                      )}{" "}
                      · {upcoming.appointment.timeSlot}
                    </p>
                    <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-accent/15 px-2.5 py-1 text-xs font-medium text-accent-foreground">
                      <Calendar className="h-3.5 w-3.5" /> Queue #
                      {upcoming.appointment.queueNumber}
                    </div>
                  </div>
                ) : (
