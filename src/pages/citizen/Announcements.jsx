...
const Announcements = () => {
    ...
    return (<>
      ...
      <div className="space-y-4">
        {list.map((a) => (
          <article key={a.id} className="rounded-2xl border border-border bg-card p-6 shadow-soft transition-all hover:shadow-elegant">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">By {a.author} · {format(new Date(a.createdAt), "MMM d, yyyy")}</p>
                </div>
              </div>
            ...
            <p className="mt-4 text-sm leading-relaxed text-foreground/90 whitespace-pre-wrap">{a.body}</p>
          </article>
        ))}
      </div>
    </>);
